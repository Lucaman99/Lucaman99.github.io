...
bijective, each $M_{\pm}$ is necessarily homeomorphic to $M$. Moreover, both are clearly open from the definition of the topology on $\widetilde{M}$. It follows that $M_{+}$ and $M_{-}$ must be
individual connected components. $\blacksquare$
...
let's discuss a few implications of the above result related to manifolds with boundary. Recall that a compact $n$ -manifold with boundary is said to be
$R$ -orientable if $M - \partial M$ is $R$ -orientable as a manifold without boundary, in the sense of the previous section. It is pretty straightforward to show that any such manifold
with boundary admits a collar neighbourhood of the boundary, which is a neighbourhood $V$ of $\partial M$ and a homeomorphism $f : V \to \partial M \times [0, 1)$ such that
$\partial M \times \{0\}$ is identified with $\partial M \subset V$ via the map. This allows us to immediately note that the inclusion $(M, \partial M) \to (M, V')$ where $V' = f^{-1}(\partial M \times [0, 1/2))$
is a homotopy equivalence, as we can collapse $V'$ to $\partial M$ and stretch $V - V'$ over $V$. It follows that
\begin{equation}
H_n(M, \partial M; R) \simeq H_n(M, V'; R) \simeq H_n(M - \partial M, V' - \partial M; R)
\end{equation}
Let $\text{Int}(M) = M - \partial M$ to simplify notation. Let $K = \text{Int}(M) - V'$, so that $\text{Int}(M) - K = V' - \partial M$. Notice that $K$ is compact,
being a closed subset of a compact manifold. We have $H_n(M - \partial M, V' - \partial M; R) = H_n(\text{Int}(M) | K; R)$. From here, we are told by the earlier lemma
that given a section $\alpha$ for the $R$ -covering space of $\text{Int}(M)$, there exists a unique $\Phi(\alpha) \in H_n(\text{Int}(M) | K; R)$ whose image is $\alpha(x) \in H_n(\text{Int}(M) | x; R)$
for all $x \in \text{Int}(M)$. The above isomorphism in turn give a unique $[M] \in H_n(M, \partial M; R)$ corresponding to the section. It is pretty easy to see that $\text{Int}(M)$ is connected if $M$ is,
hence sections are uniquely determined by their value at a single point, hence similar to earlier, we get our isomorphism $H_n(M, \partial M; R) \simeq R$.
